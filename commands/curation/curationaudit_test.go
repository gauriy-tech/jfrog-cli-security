...
func getTestCasesForDoCurationAudit() []testCase {
	tests := []testCase{
		...
		{
			// One HEAD probe 500s (fatal: only 200/403 are curation-meaningful), the
			// other 403s. The walker aborts on the fatal response and we must NOT
			// emit a partial report — that's exactly the "Found 0 blocked packages
			// + exit 1" UX the strict short-circuit in auditTree fixes. Expect:
			// the fatal error is surfaced, results map stays empty so no
			// misleading 0-blocked line is printed by Run().
