...
func BuildDependencyTree(params technologies.BuildInfoBomGeneratorParams) (dependencyTrees []*xrayUtils.GraphNode, uniqueDeps []string, err error) {
	...
	// that over build-info-go's heuristic, which can mis-identify the root
...
func probeBlockedDirectDeps(params technologies.BuildInfoBomGeneratorParams, curWd, workspaceMemberRel string) ([]blockedDirectDep, int) {
	...
	for _, name := range names {
		...
		// Diagnostic: dump the raw 403 body when policy extraction failed
		// so 'JFROG_CLI_LOG_LEVEL=DEBUG' reveals exactly why the table row
		// landed in the "could not be parsed" fallback. The probe and the
		// walker (commands/curation.getBlockedPackageDetails) share the
		// same parse pipeline; if the body came back in a different
		// envelope or stripped of policy details by a non-curation auth
		// layer, that mismatch shows up here. Only emitted on the failed
		// branch so a clean V3 audit never floods debug logs with 403
		// bodies that did parse correctly.
...
func expandYarnWorkspaceDirs(curWd string) []string {
	...
	patterns := parseYarnWorkspacesField(raw.Workspaces)
	if len(patterns) == 0 {
		return nil
	}
...
// parseYarnWorkspacesField handles both yarn V2+ workspace shapes. Returns
// nil for any shape we don't recognise (e.g. yarn V1 with workspaces in a
// "nohoist" sub-object) so the caller falls back to root-only probing.
func parseYarnWorkspacesField(raw json.RawMessage) []string {
	var arr []string
...
func printBlockedDirectDepsTable(blocked []blockedDirectDep, totalProbed int, format string) error {
	if len(blocked) == 0 {
		return nil
	}
	if format == "json" {
...
func configureYarnResolutionServerAndRunInstall(params technologies.BuildInfoBomGeneratorParams, curWd, yarnExecPath string) error {
	depsRepo := params.DependenciesRepository
	if depsRepo == "" {
		// Run install without configuring an Artifactory server
		return runYarnInstallAccordingToVersion(curWd, yarnExecPath, params.InstallCommandArgs, params.IsCurationCmd, params.OutputFormat)
	}
...
// preserve it.
func runYarnInstallAccordingToVersion(curWd, yarnExecPath string, installCommandArgs []string, isCurationCmd bool, outputFormat string) error {
	...
	if isYarnV1 {
		...
		if err != nil {
			return fmt.Errorf("failed while checking for existence of node_modules directory: %s", err.Error())
		}
		if !nodeModulesDirExists {
			defer func() {
				_ = fileutils.RemoveTempDir(nodeModulesFullPath)
			}()
		}
...
// that mark it as a yarn-managed project root. Used by
// findClaimingYarnWorkspaceRoot to disambiguate between yarn and npm
// workspaces — both ecosystems use a "workspaces" field, but only yarn
// puts these indicators next to it.
func directoryHasYarnIndicator(dir string) bool {
	for _, name := range []string{yarn.YarnLockFileName, ".yarnrc.yml", ".yarnrc", ".yarn"} {
