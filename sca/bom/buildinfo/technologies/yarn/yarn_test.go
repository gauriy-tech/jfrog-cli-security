...
// build-info-go's mis-identification of the project root when package.json
...
func TestParseYarnWorkspacesField(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			got := parseYarnWorkspacesField([]byte(tc.json))
...
func TestFindClaimingYarnWorkspaceRoot(t *testing.T) {
	...
	t.Run("npm-workspaces parent is not claimed", func(t *testing.T) {
		...
		// directoryHasYarnIndicator must reject this ancestor, otherwise
		// 'jf ca --working-dirs=packages/admin-ui' on an npm-workspaces
		// project would silently switch to the yarn code path and
		// resolve through 'yarn install' against an npm registry.
