...
func validateLockfileVersion(v string) error {
	...
	if majorNum < 9 {
		return fmt.Errorf("pnpm-lock.yaml lockfileVersion %q is not supported (only 9.0 and later, written by pnpm 9 and newer, are parseable); run 'pnpm install --lockfile-only' with pnpm %d or later to regenerate", v, minSupportedPnpmMajorVersion)
	}
	return nil
}
